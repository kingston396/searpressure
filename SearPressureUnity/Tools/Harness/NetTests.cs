...
static partial class Tests
{
    ...
    static void NetScenario(string url)
    {
        ...
        // ---- hosting and joining ----
        host.hostGame();
        Check(WaitFor(all, () => host.NET.code.Length == 4), "host gets a room code: " + host.NET.code + " / " + host.OnlineStatus + " / " + host.NET.role);
        Check(host.NET.code.All(c => Game.CODE_CHARS.IndexOf(c) >= 0), "room code uses CODE_CHARS");
        Type(guest, host.NET.code); guest.joinGame();
        Check(WaitFor(all, () => host.NET.guests.Count == 1 && guest.NET.joined), "guest joins with the code");
        Check(WaitFor(all, () => host.NET.guests.Count == 1 && host.NET.fits.Count == 2 && guest.NET.fits.Count == 2), "fits go round (host and guest see 2 chefs)");
        Check(host.screen == "scr-title" && guest.screen == "scr-online", $"host on title, guest waits on the online screen ({host.screen}/{guest.screen})");
        Check(guest.OnlineStatus.StartsWith("Connected!"), "guest status: " + guest.OnlineStatus);
        Type(guest2, host.NET.code); guest2.joinGame();
        Check(WaitFor(all, () => host.NET.guests.Count == 2 && guest2.NET.joined && guest.NET.fits.Count == 3 && guest2.NET.fits.Count == 3), "second guest joins; everyone sees 3 chefs");
        Step(all, 0.3);
        ...
        host.play(0);
        Check(host.G != null && host.G.chefs.Count == 3, "host kitchen has 3 chefs");
        Check(WaitFor(all, () => guest.G != null && guest2.G != null), "guests receive 'start'");
        Check(guest.G != null && guest.G.active == 1 && guest2.G.active == 2, $"guests drive their own chef (slots {guest.G?.active}, {guest2.G?.active})");
        Check(guest.G.lvIdx == 0 && guest.G.transposed == host.G.transposed && guest.G.cols == host.G.cols, "guest kitchen matches the host's layout");

        // A friend can't join halfway through a service.
        Type(late, host.NET.code); late.joinGame();
        Check(WaitFor(all, () => late.NET.role == null && late.OnlineStatus.Contains("middle of a service")), "late joiner turned away: " + late.OnlineStatus);
        Check(host.NET.guests.Count == 2, "host still has 2 guests");

        Check(WaitFor(all, () => host.G.phase == "play" && guest.G.phase == "play", 10), "countdown ends on host and guest");
        host.spawnOrder(); host.spawnOrder();
        host.G.score = 42;
        Step(all, 0.5);
        Check(WaitFor(all, () => guest.G.score == 42 && guest.G.orders.Count == host.G.orders.Count && guest.G.orders.Select(o => o.id).SequenceEqual(host.G.orders.Select(o => o.id))),
        ...
        Check(Math.Abs(guest.G.time - host.G.time) < 0.5, $"clock in step (guest {guest.G.time:0.0}, host {host.G.time:0.0})");
        ...
        Check(facing, "guest's chef faces a crate");
        Check(WaitFor(all, () => U.Hypot(hc.x - x0, hc.y - y0) > 0.5), $"host sees the guest chef move ({x0:0.00},{y0:0.00}) -> ({hc.x:0.00},{hc.y:0.00})");
        Check(WaitFor(all, () => hc.held != null), "host: guest chef picked up " + (hc.held != null ? hc.held.type + " " + hc.held.kind : "nothing"));
        Check(WaitFor(all, () => guest.G.chefs[1].held != null && guest.G.chefs[1].held.kind == hc.held?.kind), "guest's snapshot shows what they hold");
        var gc = guest.G.chefs[1];
        Check(U.Hypot(gc.x - hc.x, gc.y - hc.y) < 0.3, $"guest prediction close to host ({gc.x:0.00},{gc.y:0.00}) vs ({hc.x:0.00},{hc.y:0.00})");
        // Guest 2 sees chef 1 through snapshots too.
        Check(WaitFor(all, () => U.Hypot(guest2.G.chefs[1].x - hc.x, guest2.G.chefs[1].y - hc.y) < 0.3), "other guest sees chef 1 where the host has it");
        Step(all, 0.3);
        ...
        // ---- pause propagation ----
        guest.pauseGame();
        Check(guest.paused && guest.screen == "scr-pause", "guest pauses");
        Check(WaitFor(all, () => host.paused && host.screen == "scr-pause" && guest2.paused), "host and other guest pause too");
        double tPaused = host.G.time;
        Step(all, 0.5);
        Check(host.G.time == tPaused, "host clock stops while paused");
        Program.Shot(guest2, "net_guest2_paused.png");
        host.resumeGame();
        Check(WaitFor(all, () => !guest.paused && !guest2.paused && guest.screen == null), "host resumes everyone");
        Step(all, 0.3);
        Check(host.G.time < tPaused, "host clock runs again");

        // ---- a guest leaves mid-service: the host plays on ----
        guest2.pauseGame();
        Check(WaitFor(all, () => host.paused), "leaving guest pauses everyone on the way out (like the web)");
        guest2.toMenu();   // "Leave" on the pause card
        Check(guest2.NET.role == null && guest2.G == null, "leaving guest is back offline");
        Check(WaitFor(all, () => host.NET.guests.Count == 1 && !host.paused), "host drops them and unpauses");
        Check(host.G != null && host.G.phase == "play", "host keeps playing");
        Check(host.G.floats.Any(f => f.text == "A friend left"), "host shows 'A friend left'");
        double gScore = guest.G.time;
        Step(all, 0.5);
        Check(guest.G != null && guest.G.time < gScore && !guest.paused, "remaining guest keeps getting snapshots");
        Program.Shot(host, "net_host_friend_left.png");

        // ---- versus ----
        host.toMenu();   // "Pick another kitchen"
        Check(WaitFor(all, () => guest.G == null && guest.screen == "scr-online" && host.screen == "scr-title"), "back to the lobby for everyone");
        host.setMode("versus");
        Check(WaitFor(all, () => guest.NET.mode == "versus"), "guest hears the mode switch");
        Program.Shot(host, "net_host_lobby_versus.png");
        host.play(0);
        Check(WaitFor(all, () => guest.G != null && guest.G.versus), "guest gets a versus kitchen");
        Check(host.G.versus && host.G.chefs.Count == 2 && guest.G.chefs.Count == 2 && guest.G.active == 0, "each phone runs its own two-chef kitchen");
        Check(WaitFor(all, () => host.G.phase == "play" && guest.G.phase == "play", 10), "both kitchens start");
        // Same seed: same tickets.
        Step(all, 3);
        Check(host.G.orders.Select(o => o.recipe).Take(1).SequenceEqual(guest.G.orders.Select(o => o.recipe).Take(1)) && host.G.orders.Count > 0, $"same first ticket ({host.G.orders.FirstOrDefault()?.recipe} / {guest.G.orders.FirstOrDefault()?.recipe})");
        ...
        Check(hostC.x == hx && hostC.y == hy, "guest walking doesn't move the host's chefs");
        host.G.score = 30; guest.G.score = 55;
        Check(WaitFor(all, () => guest.NET.vs.TryGetValue(0, out var r) && r.score == 30 && host.NET.vs.TryGetValue(1, out var r1) && r1.score == 55), "vs board: scores go round");
        Step(all, 0.3);
        Console.WriteLine("  host summary: " + host.vsSummary());
        Console.WriteLine("  guest summary: " + guest.vsSummary());
        Check(guest.vsSummary().Contains("Waiting") && guest.vsSummary().Contains("You 55"), "guest summary while racing");
        ...
        Check(WaitFor(all, () => host.screen == "scr-results" && guest.screen == "scr-results" && host.vsSummary().StartsWith("You came 2nd") && guest.vsSummary().StartsWith("You win!"), 10), "versus results: " + guest.vsSummary());
        Step(all, 0.2);
        Program.Shot(guest, "net_guest_versus_results.png");

        // ---- the host leaves: the guest is told ----
        host.netLeave(); host.show("scr-title");
        Check(WaitFor(all, () => guest.NET.role == null && guest.screen == "scr-online"), "guest is dropped when the host leaves: " + guest.OnlineStatus);
    }
    ...
    static void RelayScenario(string url)
    {
        ...
        var host = gs[0];
        host.hostGame();
        Check(WaitFor(gs, () => host.NET.code.Length == 4), "host gets code " + host.NET.code);
        for (int i = 1; i <= 3; i++)
        {
            Type(gs[i], host.NET.code); gs[i].joinGame();
            int n = i;
            Check(WaitFor(gs, () => host.NET.guests.Count == n && gs[n].NET.joined), $"guest {i} joins");
        }
        Check(WaitFor(gs, () => gs.Take(4).All(g => g.netPlayers() == 4)), "everyone counts 4 players");
        Type(gs[4], host.NET.code); gs[4].joinGame();
        Check(WaitFor(gs, () => gs[4].NET.role == null && gs[4].OnlineStatus.Contains("full")), "5th player rejected: " + gs[4].OnlineStatus);
        Check(host.NET.guests.Count == 3, "host still has 3 guests");
        string bad = host.NET.code == "ZZZZ" ? "YYYY" : "ZZZZ";
        Type(gs[5], bad); gs[5].joinGame();
        Check(WaitFor(gs, () => gs[5].NET.role == null && gs[5].OnlineStatus.StartsWith("No kitchen found")), "unknown code rejected: " + gs[5].OnlineStatus);
        // Co-op with four: every guest gets a chef.
        host.play(0);
        Check(WaitFor(gs, () => gs.Skip(1).Take(3).All(g => g.G != null)), "all three guests start");
        Check(host.G.chefs.Count == 4 && gs[3].G.active == 3, "4 chefs, the last guest drives chef 3");
        Step(gs, 4);
        ...
        Check(WaitFor(gs, () => host.NET.guests.Count == 2), "a guest's socket closing tells the host");
        Check(host.G != null, "host keeps playing");
    }
}
