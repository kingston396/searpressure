...
**Data safety form**
- Does your app collect or share any of the required user data types? **No** (progress is saved only on the device).
- The Unity engine may send anonymous device and diagnostics information to Unity. Before you submit, check Unity's current guidance on Google Play data safety for your Unity version. If it says to declare anything, it will usually be **App info and performance → Diagnostics**: collected, not shared, used for analytics/app functionality, not linked to identity. Mark it the same way in the form.
- Is data encrypted in transit? Yes.  Can users request deletion? Not applicable (nothing collected), or answer via the contact email.
...
```
Two chefs, one food truck and a road trip across America. Chop, cook and swap chefs to beat the rush, and try a new daily challenge every day. No ads.
```
...
**App Privacy ("nutrition label"):** Data Not Collected. As with Google, check Unity's guidance for your Unity version; if it lists engine diagnostics, declare **Diagnostics → Crash/Performance data**, not linked to the user, not used for tracking.
